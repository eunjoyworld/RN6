...
const PersonStagger: FC<PersonProps> = ({person, deletePressed}) => {
  ...
  const avatarPressed = useCallback(() => {
    ...
    else Animated.sequence([...startAnimations, ...endAnimations])
    .start(toggleStarted)
  }, [started])
  ...
  const icons = useMemo(
    () => balls.map((color, index) => {
        const numberOfIcons = balls.length
          ...
          const animStyle = {
            transform: [{translateX: interpolate(animValue,
              [0, layout.width - numberOfIcons * iconSize])},
            {rotate: interpolate(animValue, ['0deg', '720deg'])}]
        }
        return (
          <AnimatedIcon key={color} style={[animStyle]}
            name="soccer-ball-o" size={iconSize} color={color} />
        )
      }), [layout.width])
