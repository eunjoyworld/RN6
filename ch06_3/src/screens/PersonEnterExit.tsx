...
const PersonEnterExit: FC<PersonProps> = ({person, deletePressed}) => {
  ...
  const iconAnimations = useMemo(
    () => iconAnimValues.map(animValue =>
        Animated.spring(animValue, {useNativeDriver: true,
          toValue: !started ? 1 : 0})),
    [started])
  ...
  const enterAnimation = useCallback(() => {
    Animated.sequence([
      Animated.timing(leftToRightAnimValue, {
        useNativeDriver: true, toValue: 1, duration: 1 * 1000,
  ...
  const exitAnimation = useCallback(() => {
    Animated.sequence([
      ...
      Animated.timing(leftToRightAnimValue, {useNativeDriver: true, toValue: 0,
        duration: 0.3 * 1000})
    ]).start(deletePressed)
  }, [started])
  ...
  return (
    <Animated.View style={[styles.view, enterLeaveTransformStyle]}>
      ...
      <View style={[styles.rightView]}>
        ...
        <Animated.Image style={[styles.image, fadeInOutStyle,
        topOrBottomTransformStyle]} source={{uri: person.image}} />
